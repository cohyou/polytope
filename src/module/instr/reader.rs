use std::io::Read;
use crate::valtype::{ValType, byte_to_valtype};
use crate::byte::Byte;
use crate::module::expr::{Expr, read_expr};
use crate::module::idx::{read_labelidx, read_labelindices, read_funcidx, read_localidx, read_globalidx};
use crate::util::read_u32_from_leb128;
use super::{Instr, BlockType, MemArg, ValSize, ValSign, ITestOp, IRelOp, FRelOp, IUnOp, IBinOp, FUnOp, FBinOp, CvtOp};


pub(in crate::module) fn read_instr(b: Byte, reader: &mut impl Read) -> Instr {
    match b {
        //////////////////////////
        ...
        0x01 => Instr::Nop,
        0x02 => {
            let block_type = read_blocktype(reader);
            let expr = read_expr(reader);
            Instr::Block(block_type, expr)
        },
        0x03 => {
            let block_type = read_blocktype(reader);
            let expr = read_expr(reader);
        ...
        0x04 => {
            ...
            loop {
                if let Some(Ok(b)) = reader.bytes().next() {
                    if b == 0x05 {
                        // else
                        instrs_true = instrs;
                        expr_false = Some(read_expr(reader));
                        break;
                    }  
                    if b == 0x08 {
                        // end
            ...
            Instr::If(block_type, Expr(instrs_true), expr_false)
        },
        0x0C => Instr::Br(read_labelidx(reader)),
        0x0D => Instr::BrIf(read_labelidx(reader)),
        0x0E => Instr::BrTable(read_labelindices(reader), read_labelidx(reader)),
        0x0F => Instr::Return,
        0x10 => Instr::Call(read_funcidx(reader)),
        0x11 => {
            let funcidx = read_funcidx(reader);
            if let Some(Ok(b)) = reader.bytes().next() {
                if b == 0x00 {
                    // 0x00 is table index (for future)
                    Instr::CallIndirect(funcidx)
                } else {
                    panic!("invaild on read_instr CALL INDIRECT");
                }
            } else {
                panic!("invaild on read_instr CALL INDIRECT");
            }
        ...
        ///////////////////////////
        0x20 => Instr::LocalGet(read_localidx(reader)),
        0x21 => Instr::LocalSet(read_localidx(reader)),
        0x22 => Instr::LocalTee(read_localidx(reader)),
        0x23 => Instr::GlobalGet(read_globalidx(reader)),
        0x24 => Instr::GlobalSet(read_globalidx(reader)),

        /////////////////////////
        // Memory Instructions //
        /////////////////////////
        0x28 => Instr::Load(ValType::I32, read_memarg(reader)),
        0x29 => Instr::Load(ValType::I64, read_memarg(reader)),
        0x2A => Instr::Load(ValType::F32, read_memarg(reader)),
        0x2B => Instr::Load(ValType::F64, read_memarg(reader)),

        0x2C => Instr::ILoad8(ValSize::V32, ValSign::S, read_memarg(reader)),
        0x2D => Instr::ILoad8(ValSize::V32, ValSign::U, read_memarg(reader)),
        0x2E => Instr::ILoad16(ValSize::V32, ValSign::S, read_memarg(reader)),
        0x2F => Instr::ILoad16(ValSize::V32, ValSign::U, read_memarg(reader)),

        0x30 => Instr::ILoad8(ValSize::V64, ValSign::S, read_memarg(reader)),
        0x31 => Instr::ILoad8(ValSize::V64, ValSign::U, read_memarg(reader)),
        0x32 => Instr::ILoad16(ValSize::V64, ValSign::S, read_memarg(reader)),
        0x33 => Instr::ILoad16(ValSize::V64, ValSign::U, read_memarg(reader)),
        0x34 => Instr::I64Load32(ValSign::S, read_memarg(reader)),
        0x35 => Instr::I64Load32(ValSign::U, read_memarg(reader)),

        0x36 => Instr::Store(ValType::I32, read_memarg(reader)),
        0x37 => Instr::Store(ValType::I64, read_memarg(reader)),
        0x38 => Instr::Store(ValType::F32, read_memarg(reader)),
        0x39 => Instr::Store(ValType::F64, read_memarg(reader)),

        0x3A => Instr::IStore8(ValSize::V32, read_memarg(reader)),
        0x3B => Instr::IStore16(ValSize::V32, read_memarg(reader)),
        0x3C => Instr::IStore8(ValSize::V64, read_memarg(reader)),
        0x3D => Instr::IStore16(ValSize::V64, read_memarg(reader)),
        ...
        0xFC => {
            ...
            let variable_length = read_u32_from_leb128(reader);
...
fn read_blocktype(reader: &mut impl Read) -> BlockType {
    // TODO: s33ではなく、u32で読んでいる
    if let Some(Ok(b)) = reader.bytes().next() {
        match b {
            0x40 => BlockType::Vt(None),
            0x7F | 0x7E | 0x7D | 0x7C => BlockType::Vt(Some(byte_to_valtype(b))),
            _ => BlockType::X(read_u32_from_leb128(reader)), 
        }
    } else {
        panic!("invalid on read_blocktype");
    }
    
}

fn read_memarg(reader: &mut impl Read) -> MemArg {
    let align = read_u32_from_leb128(reader);
    let offset = read_u32_from_leb128(reader);
